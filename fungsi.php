...
function preproses($teks) {
	...
	$restem = $con->query("SELECT * FROM tbstem ORDER BY Id");
...
function buatindex() {
	include 'koneksi.php';
		...
		$con->query("TRUNCATE TABLE tbindex");

		//ambil semua berita (teks)
		$resBerita = $con->query("SELECT * FROM tbberita ORDER BY Id");
			...
			//terapkan preprocessing
  			...
  			foreach ($aberita as $j => $value) {
				...
				if ($aberita[$j] != "") {
					...
					$rescount = $con->query("SELECT Count FROM tbindex WHERE Term = '$aberita[$j]' AND DocId = $docId");
					...
					if ($num_rows > 0) {
						...
						$con->query("UPDATE tbindex SET Count = $count WHERE Term = '$aberita[$j]' AND DocId = $docId");
					}
					//jika belum ada, langsung simpan ke tbindex
					else {
						$con->query("INSERT INTO tbindex (Term, DocId, Count) VALUES ('$aberita[$j]', $docId, 1)");
					}
				} //end if
			} //end foreach
  		} //end while
...
function hitungbobot() {
	...
	while($rowbobot = mysqli_fetch_array($resBobot)) {
		...
		$resNTerm = $con->query("SELECT Count(*) as N FROM tbindex WHERE Term = '$term'");
		...
		$resUpdateBobot = $con->query("UPDATE tbindex SET Bobot = $w WHERE Id = $id");
  	} //end while $rowbobot
...
function panjangvektor() {
	...
	while($rowDocId = mysqli_fetch_array($resDocId)) {
		...
		$resVektor = $con->query("SELECT Bobot FROM tbindex WHERE DocId = $docId");
		...
		$resInsertVektor = $con->query("INSERT INTO tbvektor (DocId, Panjang) VALUES ($docId, $panjangVektor)");
	} //end while $rowDocId
...
function hitungsim($query) {
	...
	$resn = $con->query("SELECT Count(*) as n FROM tbvektor");
	...
	for ($i=0; $i<count($aquery); $i++) {
		...
		$resNTerm = $con->query("SELECT Count(*) as N from tbindex WHERE Term = '$aquery[$i]'");
		$rowNTerm = mysqli_fetch_array($resNTerm);
		$NTerm = $rowNTerm['N'] ;
    //$idf = 0;
    $idf = 0;
	...
	$resDocId = $con->query("SELECT * FROM tbvektor ORDER BY DocId");
	while ($rowDocId = mysqli_fetch_array($resDocId)) {
		...
		$resTerm = $con->query("SELECT * FROM tbindex WHERE DocId = $docId");
		...
		if ($dotproduct > 0) {
			...
			$resInsertCache = $con->query("INSERT INTO tbcache (Query, DocId, Value) VALUES ('$query', $docId, $sim)");
			$jumlahmirip++;
		}
	...
	if ($jumlahmirip == 0) {
		$resInsertCache = $con->query("INSERT INTO tbcache (Query, DocId, Value) VALUES ('$query', 0, 0)");
	}
...
function ambilcache($keyword) {
	include "koneksi.php";
	$resCache = $con->query("SELECT *  FROM tbcache WHERE Query = '$keyword' ORDER BY Value DESC");
	...
	if ($num_rows >0) {
		...
		while ($rowCache = mysqli_fetch_array($resCache)) {
			...
			if ($docId != 0) {
				...
				$resBerita = $con->query("SELECT * FROM tbberita WHERE Id = $docId");
